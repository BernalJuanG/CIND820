...
#read.csv() method which is older,slower,and converts some character variables to factors.
#airbnb<-read.csv("./AB_NYC_2019.csv",sep=",",header=TRUE,na.strings=c(""," ","NA"))

#read_csv() method which is a newer,faster,method that outputs a tibble and does not convert the data unecessarily.
...
##It's really just last_review and reviews_per_month contributing all NA's. The rest of the data doesn't have any. The nice thing is that at least reviews_per_month is a numerical value so we can maybe impute with an average.
...
```{r}
#Let's have a look at the rows with the NA's. Might these be input errors? Or should they actually be 0's? Or maybe it really is just missing data.
...
#I'm just going to drop that last_review column for now.
...
#A look at the original data shows that all of these listings with an abnormally high calculated_host_listings_count come from a host under the name of "Sonder (NYC)". This is probably a large company who manages hundreds of listings in NYC. It seems valuable to me to keep the data from these kinds of hosts because they reflect the fact that in NY the market is not simple made up of individuals.

#This code takes all the listings in the ORIGINAL dataset(before we removed the host_name) where the calculated_host_listing_count is the max (327) and returns the unique host_name values.There is only one.
unique(
  airbnb %>%
...
#Leaving it as is for now, Im not sure
```
...
```{r}
dmy<-dummyVars("~.",data=airbnb_df[,c(1,5)],fullRank=T)
